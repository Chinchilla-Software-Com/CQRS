		...
		, IBlobStorageSnapshotStoreConnectionStringFactory
	{
		...
		public override IEnumerable<string> GetWritableConnectionStrings()
		{
			...
			try
			{
				...
				string blobStorageWritableSnapshotStoreConnectionString = ConfigurationManager.GetSetting(BlobStorageWritableSnapshotStoreConnectionStringKey);
				if (string.IsNullOrWhiteSpace(blobStorageWritableSnapshotStoreConnectionString))
				{
					Logger.LogDebug(string.Format("No application setting named '{0}' in the configuration file.", BlobStorageWritableSnapshotStoreConnectionStringKey), "BlobStorageSnapshotStoreConnectionStringFactory\\GetWritableConnectionStrings");
					blobStorageWritableSnapshotStoreConnectionString = ConfigurationManager.GetSetting(BlobStorageSnapshotStoreConnectionStringKey);
				}
				...
				while (!string.IsNullOrWhiteSpace(blobStorageWritableSnapshotStoreConnectionString))
				{
					...
					blobStorageWritableSnapshotStoreConnectionString = ConfigurationManager.GetSetting(string.Format("{0}.{1}", BlobStorageWritableSnapshotStoreConnectionStringKey, writeIndex));
			...
			{
				throw new MissingApplicationSettingException(BlobStorageSnapshotStoreConnectionStringKey, string.Format("No application settings named '{0}' was found in the configuration file with the cloud storage connection string.", BlobStorageSnapshotStoreConnectionStringKey), exception);
			}
			finally
			{
		...
		public override string GetReadableConnectionString()
		{
			...
			try
			{
				string blobStorageWritableSnapshotStoreConnectionString = ConfigurationManager.GetSetting(BlobStorageReadableSnapshotStoreConnectionStringKey);
				if (string.IsNullOrWhiteSpace(blobStorageWritableSnapshotStoreConnectionString))
				{
					Logger.LogDebug(string.Format("No application setting named '{0}' in the configuration file.", BlobStorageReadableSnapshotStoreConnectionStringKey), "BlobStorageSnapshotStoreConnectionStringFactory\\GetReadableConnectionStrings");
					blobStorageWritableSnapshotStoreConnectionString = ConfigurationManager.GetSetting(BlobStorageSnapshotStoreConnectionStringKey);
				}
