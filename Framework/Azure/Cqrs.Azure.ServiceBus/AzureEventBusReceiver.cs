...
namespace Cqrs.Azure.ServiceBus
{
...
#endif
	{
		...
		public virtual
#if NETSTANDARD2_0 || NET48_OR_GREATER
			async Task RegisterGlobalEventHandlerAsync<TMessage>(Func<TMessage, Task>
#else
			void RegisterGlobalEventHandler<TMessage>(Action<TMessage>
#endif
			handler, bool holdMessageLock = true)
			where TMessage : IMessage
		{
...
#endif
			(TelemetryHelper, Routes, handler, holdMessageLock);
		}
		 ...
		 serviceBusReceiver, BrokeredMessage message)
		{
			...
			string telemetryName = $"Cqrs/Handle/Event/{message.MessageId}";
			...
			try
			{
				try
				{
					Logger.LogDebug($"A event message arrived with the id '{message.MessageId}'.");
					...
					IEvent<TAuthenticationToken> @event =
						...
						{
							wasSuccessfull = null;
							telemetryName = $"Cqrs/Handle/Event/Skipped/{message.MessageId}";
							responseCode = "204";
							...
							{
								throw new MessageLockLostException($"The lock supplied for the skipped event message '{message.MessageId}' is invalid.", exception);
							}
#endif
...
#endif
								brokeredMessageRenewCancellationTokenSource, message, "event");
						}
					);
					...
					if (wasSuccessfull != null)
					{
						if (@event != null)
						{
							...
							telemetryName = $"Cqrs/Handle/Event/{telemetryName}";
						}
						// Remove message from queue
						try
						{
						...
						{
							throw new MessageLockLostException($"The lock supplied for event '{@event.Id}' of type {@event.GetType().Name} is invalid. To avoid this issue add a '{@event.GetType().FullName}.ShouldRefresh' application settings", exception);
						}
#endif
					}
			...
			catch (UnAuthorisedMessageReceivedException exception)
			{
				...
				// Indicates a problem, unlock message in queue
				Logger.LogError($"An event message arrived with the id '{message.MessageId}' but was not authorised.", exception: exception);
				wasSuccessfull = false;
				responseCode = "401";
			...
			catch (NoHandlersRegisteredException exception)
			{
				TelemetryHelper.TrackException(exception, null, telemetryProperties);
				// Indicates a problem, unlock message in queue
				Logger.LogError($"An event message arrived with the id '{message.MessageId}' but no handlers were found to process it.", exception: exception);
				wasSuccessfull = false;
				responseCode = "501";
				telemetryProperties.Add("ExceptionType", exception.GetType().FullName);
				telemetryProperties.Add("ExceptionMessage", exception.Message);
			...
			catch (NoHandlerRegisteredException exception)
			{
				TelemetryHelper.TrackException(exception, null, telemetryProperties);
				// Indicates a problem, unlock message in queue
				Logger.LogError($"An event message arrived with the id '{message.MessageId}' but no handler was found to process it.", exception: exception);
				wasSuccessfull = false;
				responseCode = "501";
				telemetryProperties.Add("ExceptionType", exception.GetType().FullName);
				telemetryProperties.Add("ExceptionMessage", exception.Message);
		...
		public virtual void Start()
		{
			...
			SafeTask.RunSafelyAsync(async () => {
				...
				await RegisterReceiverMessageHandlerAsync(ReceiveEventAsync, options);
			}).Wait();
#else
