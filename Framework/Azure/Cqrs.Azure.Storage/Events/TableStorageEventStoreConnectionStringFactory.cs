		...
		: ITableStorageStoreConnectionStringFactory
	{
		...
		public virtual IEnumerable<string> GetWritableConnectionStrings()
		{
			...
			try
			{
				...
				string tableStorageWritableEventStoreConnectionString = ConfigurationManager.GetConnectionString(TableStorageWritableEventStoreConnectionStringKey) ?? ConfigurationManager.GetSetting(TableStorageWritableEventStoreConnectionStringKey);
				if (string.IsNullOrWhiteSpace(tableStorageWritableEventStoreConnectionString))
				{
					Logger.LogDebug($"No application setting named '{TableStorageWritableEventStoreConnectionStringKey}' in the configuration file.", "TableStorageEventStoreConnectionStringFactory\\GetWritableConnectionStrings");
					tableStorageWritableEventStoreConnectionString = ConfigurationManager.GetConnectionString(TableStorageEventStoreConnectionStringKey) ?? ConfigurationManager.GetSetting(TableStorageEventStoreConnectionStringKey);
				}
				...
				while (!string.IsNullOrWhiteSpace(tableStorageWritableEventStoreConnectionString))
				{
					...
					tableStorageWritableEventStoreConnectionString = ConfigurationManager.GetConnectionString($"{TableStorageWritableEventStoreConnectionStringKey}.{writeIndex}") ?? ConfigurationManager.GetSetting($"{TableStorageWritableEventStoreConnectionStringKey}.{writeIndex}");

					writeIndex++;
				}

				if (!collection.Any())
					throw new MissingConnectionStringException(TableStorageEventStoreConnectionStringKey);

				return collection;
			}
			catch (NullReferenceException exception)
			{
				throw new MissingConnectionStringException(TableStorageEventStoreConnectionStringKey, exception);
			}
			finally
			{
		...
		public virtual string GetReadableConnectionString()
		{
			...
			try
			{
				string tableStorageWritableEventStoreConnectionString = ConfigurationManager.GetConnectionString(TableStorageReadableEventStoreConnectionStringKey) ?? ConfigurationManager.GetSetting(TableStorageReadableEventStoreConnectionStringKey);
				if (string.IsNullOrWhiteSpace(tableStorageWritableEventStoreConnectionString))
				{
					Logger.LogDebug($"No application setting named '{TableStorageReadableEventStoreConnectionStringKey}' in the configuration file.", "TableStorageEventStoreConnectionStringFactory\\GetReadableConnectionStrings");
					tableStorageWritableEventStoreConnectionString = ConfigurationManager.GetConnectionString(TableStorageEventStoreConnectionStringKey) ?? ConfigurationManager.GetSetting(TableStorageEventStoreConnectionStringKey);
				}
