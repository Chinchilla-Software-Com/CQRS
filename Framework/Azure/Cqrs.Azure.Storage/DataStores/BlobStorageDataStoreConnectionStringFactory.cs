	...
	public class BlobStorageDataStoreConnectionStringFactory : IBlobStorageDataStoreConnectionStringFactory
	{
		...
		public virtual IEnumerable<string> GetWritableConnectionStrings()
		{
			...
			try
			{
				...
				string blobStorageWritableDataStoreConnectionString = ConfigurationManager.GetConnectionString(BlobStorageWritableDataStoreConnectionStringKey) ?? ConfigurationManager.GetSetting(BlobStorageWritableDataStoreConnectionStringKey);
				if (blobStorageWritableDataStoreConnectionString == null)
				{
					Logger.LogDebug($"No application settings named '{BlobStorageWritableDataStoreConnectionStringKey}' was found in the configuration file with the cloud storage connection string.", "BlobStorageDataStoreConnectionStringFactory\\GetWritableConnectionStrings");
					blobStorageWritableDataStoreConnectionString = ConfigurationManager.GetConnectionString(BlobStorageDataStoreConnectionStringKey) ?? ConfigurationManager.GetSetting(BlobStorageDataStoreConnectionStringKey);
				}
				...
				while (!string.IsNullOrWhiteSpace(blobStorageWritableDataStoreConnectionString))
				{
					...
					blobStorageWritableDataStoreConnectionString = ConfigurationManager.GetConnectionString($"{BlobStorageWritableDataStoreConnectionStringKey}.{writeIndex}") ?? ConfigurationManager.GetSetting($"{BlobStorageWritableDataStoreConnectionStringKey}.{writeIndex}");
					writeIndex++;
				}

				if (!collection.Any())
					throw new MissingConnectionStringException(BlobStorageDataStoreConnectionStringKey);

				return collection;
			}
			catch (NullReferenceException exception)
			{
				throw new MissingConnectionStringException(BlobStorageDataStoreConnectionStringKey, exception);
			}
			finally
			{
		...
		public virtual string GetReadableConnectionString()
		{
			...
			try
			{
				string blobStorageWritableDataStoreConnectionString = ConfigurationManager.GetConnectionString(BlobStorageReadableDataStoreConnectionStringKey) ?? ConfigurationManager.GetSetting(BlobStorageReadableDataStoreConnectionStringKey);
				if (blobStorageWritableDataStoreConnectionString == null)
				{
					Logger.LogDebug($"No application settings named '{BlobStorageDataStoreConnectionStringKey}' was found in the configuration file with the cloud storage connection string.", "BlobStorageDataStoreConnectionStringFactory\\GetReadableConnectionStrings");
					blobStorageWritableDataStoreConnectionString = ConfigurationManager.GetConnectionString(BlobStorageDataStoreConnectionStringKey) ?? ConfigurationManager.GetSetting(BlobStorageDataStoreConnectionStringKey);
				}

				if (string.IsNullOrWhiteSpace(blobStorageWritableDataStoreConnectionString))
					throw new MissingConnectionStringException(BlobStorageReadableDataStoreConnectionStringKey);

				return blobStorageWritableDataStoreConnectionString;
			}
			catch (NullReferenceException exception)
			{
				throw new MissingConnectionStringException(BlobStorageReadableDataStoreConnectionStringKey, exception);
			}
			finally
			{
