...
namespace CQRSTests.WriteModel
{
	public class When_item_checked_in : Specification<InventoryItem, InventoryCommandHandlers, CheckInItemsToInventory>
	{
		...
		{
			return new InventoryCommandHandlers(UnitOfWork);
		}

		protected override IEnumerable<IEvent<ISingleSignOnToken>> Given()
		{
			_guid = Guid.NewGuid();
			return new List<IEvent<ISingleSignOnToken>> { new InventoryItemCreated(_guid, "Jadda") { Version = 1 }, new ItemsCheckedInToInventory(_guid, 2) { Version = 2 } };
		}
