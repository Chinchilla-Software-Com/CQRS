namespace KendoUI.Northwind.Dashboard.Code.Commands
{
	...
	/// <summary>
	/// A <see cref="ICommand{TAuthenticationToken}"/> that requests a new order gets created.
	/// </summary>
	public class CreateOrderCommand : ICommand<string>
	{
		...
		public CreateOrderCommand(Guid rsn, int orderId, string customerId, int? employeeId, DateTime? orderDate, DateTime? requiredDate, DateTime? shippedDate, int? shipViaId, decimal? freight, string shipName, string shipAddress, string shipCity, string shipRegion, string shipPostalCode, string shipCountry)
		{
			Rsn = rsn;
			OrderId = orderId;
		...
		/// <summary>
		/// Explicit cast of <see cref="OrderViewModel"/> to <see cref="CreateOrderCommand"/>
		/// </summary>
		...
		public static explicit operator CreateOrderCommand(OrderViewModel order)
		{
